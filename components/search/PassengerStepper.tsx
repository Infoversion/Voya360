  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
      ...
      <Text style={{
        ...
        fontSize: fontSize.body, fontWeight: '700', color: colors.text,
      }}>
        {value}
      </Text>
...
export function PassengerStepper({ value, onChange }: Props) {
  ...
  const decrement = (key: keyof PassengerCounts) => {
    ...
    const next = Math.max(min, value[key] - 1);
    onChange({ [key]: next });
  };
  ...
  const maxFor = (key: keyof PassengerCounts) => {
    if (key === 'infants') return Math.min(value.adults, 9 - total + value.infants);
    return 9 - total + value[key];
  };
  ...
  return (
    <View>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
        <Text style={{ fontSize: fontSize.label, color: colors.textMuted, fontWeight: '500' }}>
          Passengers
        </Text>
